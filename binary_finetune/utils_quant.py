...
class QuantizeLinear(nn.Linear):
    ...
    def __init__(self, *kargs, clip_val=2.5, weight_bits=8, input_bits=8, learnable=False, symmetric=True,
        ...
        # self.use_sa = False
        self.sa = nn.PReLU(self.in_features)
    ...
    def forward(self, input):
        ...
        # if self.symmetric and len(input.shape)==3 and self.use_sa:  ###############################################
...
class HalfQuantizeLinear(nn.Linear):

    def __init__(self, *kargs, clip_val=2.5, weight_bits=8, input_bits=8, learnable=False, symmetric=True,
                 weight_layerwise=True, input_layerwise=True, weight_quant_method="twn", input_quant_method="uniform",
                 **kwargs):
        super(HalfQuantizeLinear, self).__init__(*kargs, **kwargs)
        self.weight_bits = weight_bits
        self.input_bits = input_bits
        self.learnable = learnable
        self.symmetric = symmetric
