...
class BertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        # ----------------------------------------------------------------
        self.k_p = nn.Linear(config.hidden_size, 1)
        self.q_p = nn.Linear(config.hidden_size, 1)

        self.k_m = nn.Linear(config.hidden_size, 1)
        self.q_m = nn.Linear(config.hidden_size, 1)
