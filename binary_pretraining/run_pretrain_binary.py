...
from bipft import BinaryBertForPreTraining
from learner import KDLearner
...
def main():
    ...
    send_example_telemetry("run_mlm_no_trainer_binary_revised", args)
    ...
    teacher_model = BertForPreTraining.from_pretrained(args.model_name_or_path, config=teacher_config)
    model = BinaryBertForPreTraining.from_pretrained(args.model_name_or_path, config=student_config)
    # model = AutoModelForPreTraining.from_config(student_config)
    ...
    learner = KDLearner(model, teacher_model)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                if step%100==1:
                    loss_ = loss.mean()
                    cls_dist_loss_ = loss_dict['cls_dist_loss'].mean()
                    rep_dist_loss_ = loss_dict['rep_dist_loss'].mean()
                    ...
                    if args.with_tracking:
                        accelerator.log(
                            {
                                "total_loss": loss_,
                                "cls_dist_loss": cls_dist_loss_,
                                "rep_dist_loss": rep_dist_loss_,
